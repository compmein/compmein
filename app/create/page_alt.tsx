...
export default function CreatePage() {
   ...
   * - Quick（nano）：压到 ≤1MP（保持 AR，与 master 同构图）
   * - Pro：吃满 4MP master（<=4MP）
   */
  function exportAiStageDataUrl(modelType: "nano" | "pro") {
    if (modelType === "pro") return exportStageDataUrl(1);

    // Quick target: <= 1.00MP
    const targetPixels = 1_000_000;
    const masterPixels = stageSize.w * stageSize.h;
    if (!masterPixels) return exportStageDataUrl(1);

    // toDataURL pixelRatio scales output size: (w*pr) x (h*pr)
    // Choose pr so that (masterPixels * pr^2) <= targetPixels.
    // Subtract a tiny epsilon to avoid rounding pushing over 1MP.
    const pr = Math.min(1, Math.sqrt(targetPixels / masterPixels) - 0.001);
    return exportStageDataUrl(Math.max(0.05, pr));
  }
...
${combinePrompt}`;
      ...
      fd.append("imageSize", modelType === "pro" ? "SAFE_4MP" : "SAFE_1MP");
