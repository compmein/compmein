...
export default function CreatePage() {
  ...
  // ✅ Token balance (stored locally for now)
  ...
  function persistTokenBalance(next: number) {
    const safe = Math.max(0, Math.floor(next));
    setTokenBalance(safe);
    try {
      localStorage.setItem("tokenBalance", String(safe));
    } catch {
      // ignore
    }
  }
  ...
  function spendTokens(cost: number) {
    setTokenBalance((prev) => {
      ...
      try {
        localStorage.setItem("tokenBalance", String(next));
      } catch {
        // ignore
      }
      return next;
