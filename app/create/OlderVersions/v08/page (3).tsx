...
/** ✅ 安全写入 localStorage（避免 QuotaExceededError 直接炸页面） */
function safeSetLocalStorage(key: string, value: string, maxLen = 4_000_000) {
  try {
    ...
    console.warn(`safeSetLocalStorage failed: ${key}`, e);
    return false;
  }
}
...
export default function CreatePage() {
  ...
  async function onSelectBgFile(file: File) {
    ...
    setBgSrc(dataUrl);
    // ✅ 防止 localStorage 配额超限导致页面报错
    safeSetLocalStorage("lastBgDataUrl", dataUrl);
  }
  ...
  async function onSelectReferenceFile(file: File) {
    ...
    // ✅ 不让 localStorage 超额把页面炸掉
    safeSetLocalStorage("lastReferenceDataUrl", dataUrl);
  }
...
fd.append("modelType", modelType);
      ...
      // 保存 AI 输出（至少 1 张）
      safeSetLocalStorage("lastAIOutputDataUrl", outDataUrl);
    } catch (e: any) {
      console.error(e);
      const msg = typeof e?.message === "string" && e.message.trim() ? e.message : "Advanced Combine 失败：请看 Console / Network";
      alert(msg);
    } finally {
                ...
                <div className="lg:col-span-2 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isCombining ? "AI 合成中..." : "Quick Generation"}
                      </button>
                      ...
                      >
                        {isCombining ? "AI 合成中..." : "Pro Generation"}
                      </button>
                    </div>
