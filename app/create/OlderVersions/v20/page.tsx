...
// =======================
// ✅ Aspect ratio (Gemini allowed) + SAFE SIZE MAP (≤ ~1MP)
// =======================
...
function computeInitialPersonTransform(imgW: number, imgH: number, stageW: number, stageH: number) {
  // Default: person occupies ~65% of stage height, while respecting width.
  const targetH = stageH * 0.65;
  const maxW = stageW * 0.55;

  const s1 = targetH / Math.max(1, imgH);
  const s2 = maxW / Math.max(1, imgW);
  const scale = clamp(Math.min(s1, s2), 0.05, 3);

  const x = (stageW - imgW * scale) / 2;
  // Slightly above the bottom so feet stay inside frame
  const y = stageH - imgH * scale - stageH * 0.08;
...
export default function CreatePage() {
  ...
  useEffect(() => {
    (async () => {
      // Prefer preview (lastPersonDataUrl) for faster canvas display
      const savedPerson = localStorage.getItem("lastPersonDataUrl");

      // Cutout may be stored in localStorage or IndexedDB
      let savedCutout: string | null = localStorage.getItem("lastCutPngDataUrl");
      try {
        const where = localStorage.getItem("lastCutPngWhere");
        if (!savedCutout && where === "idb") {
          const blob = await idbGet<Blob>("lastCutPngBlob");
          if (blob) savedCutout = await blobToDataUrl2(blob);
        }
      } catch {
        // ignore
      }

      const pick = savedPerson || savedCutout;
      if (!pick) return;
      ...
      img.onload = () => {
        ...
        setPerson(computeInitialPersonTransform(iw, ih, stageSize.w, stageSize.h));
        setPersonImg(img);
      };
      img.src = pick;
    })();
  ...
  async function onSelectPersonFile(file: File) {
    // ✅ Cost-aware: keep a "cutout input" master ≤ 2048px long side (enough detail, avoids huge cutout cost)
    const master = await resizeImageFile(file, 2048, 0.9);
    setOriginalFile(master);

    // ✅ Preview on canvas: keep ≤ 1024px long side (fast + consistent with 1MP stage)
    const preview = await resizeImageFile(file, 1024, 0.9);
    ...
    try {
      ...
      localStorage.removeItem("lastCutPngWhere");
    } catch {}

    setStoredPersonDataUrl(previewDataUrl);
    safeSetLocalStorageDataUrl("lastPersonDataUrl", previewDataUrl);

    const img = new window.Image();
    img.onload = () => {
      // Auto-fit into canvas so it won't appear "outside" even if the source is large
      const iw = img.naturalWidth || img.width;
      const ih = img.naturalHeight || img.height;
      setPerson(computeInitialPersonTransform(iw, ih, stageSize.w, stageSize.h));
      setPersonImg(img);
  ...
  /**
   * ✅ 上传背景：先读原图 → pick AR → 用该 AR 的目标长边压缩（避免宽屏被放大变糊）
   * - 仍然保持“导出/AI 输入”和 canvas 完全一致
   */
  async function onSelectBgFile(file: File) {
    ...
    const targetMaxSide = Math.max(targetStage.w, targetStage.h); // 例如 16:9=1280, 21:9=1472

    const resized = await resizeImageFile(file, targetMaxSide, 0.9);
  ...
  async function doRemoveBg() {
    ...
    try {
      ...
      const blob = await resp.blob(); // PNG (transparent)

      // ✅ Keep cutout download "good enough" but not huge: cap to ≤ 2048px long side
      const cutFile = new File([blob], "cutout.png", { type: "image/png" });
      const cut2048 = await fileToDataUrlMaxSide(cutFile, 2048, "png", 0.95);
      const cut1024 = await fileToDataUrlMaxSide(cutFile, 1024, "png", 0.95);

      const cutDataUrl2048 = cut2048.dataUrl;
      const cutPreviewDataUrl = cut1024.dataUrl;

      setCutoutDataUrl(cutDataUrl2048);

      // Persist cutout safely (LS for small, IDB for big)
      try {
        if (cutDataUrl2048.length < 1_500_000) {
          safeSetLocalStorageDataUrl("lastCutPngDataUrl", cutDataUrl2048);
          localStorage.setItem("lastCutPngWhere", "ls");
          await idbDel("lastCutPngBlob");
        } else {
          const cutBlob = await dataUrlToBlob(cutDataUrl2048);
          await idbSet("lastCutPngBlob", cutBlob);
          localStorage.setItem("lastCutPngWhere", "idb");
          localStorage.removeItem("lastCutPngDataUrl");
        }
      } catch {}

      // ✅ Canvas uses smaller preview for responsiveness
      setStoredPersonDataUrl(cutPreviewDataUrl);
      safeSetLocalStorageDataUrl("lastPersonDataUrl", cutPreviewDataUrl);

      const img2 = new window.Image();
      img2.onload = () => {
        const iw = img2.naturalWidth || img2.width;
        const ih = img2.naturalHeight || img2.height;
        setPerson(computeInitialPersonTransform(iw, ih, stageSize.w, stageSize.h));
        setPersonImg(img2);
      };
      img2.src = cutPreviewDataUrl;
  ...
  /** ✅ 导出（不含 reference 面板）— pixelRatio=1 */
  ...
  /** ✅ 导出给 Nano Banana 的输入图（Image 1）— 与 Regular Comp 完全一致 */
  function exportAiStageDataUrl() {
    return exportCustomerDataUrl();
  }
  ...
  async function downloadCutout() {
    ...
    try {
      ...
      if (where === "idb") {
        ...
        if (blob) {
          const dataUrl = await blobToDataUrl2(blob);
          downloadDataUrl(dataUrl, "cutout_person1.png");
          return;
        }
      }
    } catch {
  ...
  async function runAdvancedCombine(modelType: "nano" | "pro") {
    ...
    try {
      ...
      const fd = new FormData();
      fd.append("image", sceneBlob, "scene.png");
...
${combinePrompt}`;
      ...
      // 语义保持：你后端若用这个字段也能对齐（现在是“≤1MP 安全尺寸”）
      fd.append("imageSize", "SAFE_1MP");
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          ...
          <br />
          ✅ 画布尺寸采用「安全版表」：控制在 ≤1MP 左右（避免踩更高计费档）。
          <br />
          ...
          <div className="space-y-4 lg:w-[360px] lg:shrink-0">
            ...
            <details open className="rounded-2xl border p-4">
                ...
                <div className="mt-2 space-y-2">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="mt-1 text-xs text-neutral-500">
                      上传背景会自动保存 1 张（刷新后还在）。比例会匹配 Gemini 允许列表，背景无留白铺满（居中裁切）。
                    </div>
              ...
              <div className="mt-4 rounded-2xl border p-4">
                ...
                <div className="mt-1 text-xs text-neutral-500">导出当前画布合成图</div>
                <button
            ...
            <details open className="rounded-2xl border bg-white p-4">
              ...
              <div className="mt-3 space-y-3">
                ...
                {/* ✅ 修复：reference 预览块结构（flex-col + mt-auto 生效） */}
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1 min-w-0">
                ...
                <div className="mb-2 flex items-center gap-3 text-xs text-neutral-600">
                  ...
                  <span className="ml-2 shrink-0 text-neutral-400">（默认 60%）</span>
                </div>
