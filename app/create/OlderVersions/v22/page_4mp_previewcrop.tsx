...
export default function CreatePage() {
  ...
  // ✅ 预览窗口：固定裁切框 + 可拖拽平移（仅影响显示，不影响导出/AI）
  const previewBoxRef = useRef<HTMLDivElement | null>(null);
  const [previewBoxW, setPreviewBoxW] = useState<number>(0);
  const [previewPan, setPreviewPan] = useState<{ x: number; y: number }>({ x: 0, y: 0 });
  const panDragRef = useRef<{ active: boolean; startX: number; startY: number; panX: number; panY: number } | null>(null);
...
${combinePrompt}`;
  ...
  // ✅ 预览框固定高度：不随缩放撑大（固定裁切窗口）
  const PREVIEW_BOX_H = 520;
// ✅ 监听预览区域宽度（用于裁切窗口 & 平移边界计算）
  useEffect(() => {
    if (!previewBoxRef.current) return;
    const el = previewBoxRef.current;
    const ro = new ResizeObserver(() => {
      setPreviewBoxW(el.clientWidth || 0);
    });
    ro.observe(el);
    setPreviewBoxW(el.clientWidth || 0);
  ...
  // ✅ 当缩放/画布尺寸变化时，夹紧平移，避免拖到看不到画面
  useEffect(() => {
    const viewportW = previewBoxW > 0 ? previewBoxW : 0;
    const viewportH = PREVIEW_BOX_H;
    if (viewportW <= 0) return;
    const scaledW = stageSize.w * previewScale;
    const scaledH = stageSize.h * previewScale;
  ...
  }, [previewScale, stageSize.w, stageSize.h, previewBoxW]);
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          ...
          <br />
          ✅ 导出/发送：除抠图 PNG 外，其余一律 JPG（Quality 90）。
        </p>
          ...
          <div className="space-y-4 lg:w-[360px] lg:shrink-0">
            ...
            <details open className="rounded-2xl border p-4">
              ...
              <div className="mt-4 rounded-2xl border p-4">
                ...
                <div className="mt-1 text-xs text-neutral-500">导出当前画布合成图（JPG q=0.90）</div>
                <button
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1 min-w-0">
                ...
                <div className="mb-2 flex items-center gap-3 text-xs text-neutral-600">
                  ...
                  <span className="ml-2 shrink-0 text-neutral-400">（默认 60%，拖动预览可平移，双击复位）</span>
                </div>

                {/* ✅ 预览容器：固定尺寸裁切窗口（crop）+ 可拖拽平移（仅显示） */}
                <div
                  ref={previewBoxRef}
                  className="rounded-2xl border bg-neutral-100 overflow-hidden relative"
                  style={{ height: PREVIEW_BOX_H }}
                >
                  {/* viewport：裁切窗口（宽=容器宽，高=固定高度） */}
                  <div
                    ...
                    onPointerMove={(e) => {
                      ...
                      const maxPanX = viewportW > 0 ? Math.max(0, (scaledW - viewportW) / 2) : 0;
                      const maxPanY = Math.max(0, (scaledH - viewportH) / 2);

                      setPreviewPan({
                        x: clamp(st.panX + dx, -maxPanX, maxPanX),
                        y: clamp(st.panY + dy, -maxPanY, maxPanY),
                      });
                    }}
                    onPointerUp={() => {
                    ...
                    onDoubleClick={() => setPreviewPan({ x: 0, y: 0 })}
                    title="拖动预览画面可平移（双击复位）"
                  >
                    {/* 内容：绝对居中 + 平移 + 缩放，超出部分直接被裁切 */}
                    <div
                    ...
                    >
                      <Stage width={stageSize.w} height={stageSize.h} ref={stageRef}>
                        <Layer><div className="mt-2 text-xs text-neutral-500">
