...
// =======================
// ✅ Aspect ratio (Gemini allowed) + 1K canvas
// =======================
const MAX_STAGE_LONG = 1024;
...
function stageSizeFromAllowedAR(ar: AllowedAR) {
  const r = aspectToNumber(ar);
  if (r >= 1) {
    const w = MAX_STAGE_LONG;
    const h = Math.max(1, Math.round(w / r));
    return { w, h };
  } else {
    const h = MAX_STAGE_LONG;
...
export default function CreatePage() {
  ...
  async function onSelectPersonFile(file: File) {
    const resized = await resizeImageFile(file, 1024, 0.9); // ✅ <=1K
  ...
  /** ✅ 上传背景：按 1K 压缩，并保留 1 张（localStorage）；比例自动匹配 Gemini 允许列表 */
  async function onSelectBgFile(file: File) {
    const resized = await resizeImageFile(file, 1024, 0.9); // ✅ 1K
    const dataUrl = await blobToDataURL(resized);

    const img = await loadImageFromDataUrl(dataUrl);
    const ar = pickClosestAspectRatio(img.width, img.height);
    setStageAR(ar);
    setStageSize(stageSizeFromAllowedAR(ar));
    try {
      localStorage.setItem("lastStageAR", ar);
  ...
  /** ✅ 导出（不含 reference 面板）— stage 已是 ≤1K，pixelRatio=1 */
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          ✅ Canvas / Regular Comp / Nano Banana Image 1 完全一致（≤1K），无留白（背景会自动居中裁切）。
          <br />
          ✅ Gemini aspect_ratio 与画布严格一致（自动从允许列表中选“最接近”的比例：{ALLOWED_ASPECTS.join(", ")}）。
        </p>
                ...
                <div className="mt-2 space-y-2">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="mt-1 text-xs text-neutral-500">
                      上传背景会自动保存 1 张（刷新后还在）。比例会自动匹配 Gemini 允许列表，背景会无留白铺满（居中裁切）。
                    </div>

                    <div className="mt-2 text-xs text-neutral-600">
                      当前画布：<span className="font-medium">{stageSize.w}×{stageSize.h}</span> · Gemini aspect_ratio：
                      <span className="font-medium"> {stageAR}</span>
                    </div>
                  </div>
                </div>
              </details>
  ...
  <div className="mt-3 rounded-2xl border-4 border-black bg-white p-4 overflow-hidden">
    <div className="text-center text-sm font-semibold">Selected for Clothing Reference</div>
      ...
      <button
        ...
        className="mt-auto w-full rounded-xl border px-4 py-2 text-sm hover:bg-neutral-50"
        ...
        Remove reference
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="pt-1 text-xs text-neutral-600">
                    发送给 Gemini：aspect_ratio=<span className="font-medium">{stageAR}</span>（与画布严格一致）
                  </div>
                </div>
              </div>
            </details>
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1 min-w-0">
                ...
                >
                  {/* scaled box (用于 flex 真正居中) */}
                  <div style={{ width: scaledStage.w, height: scaledStage.h }}>
                    {/* 内部用 transform scale 保持导出仍是 1K */}
                    <div
                      style={{
                ...
                <div className="mt-2 text-xs text-neutral-500">
                  说明：这里只是“显示缩放”，导出 / AI 输入仍然是 {stageSize.w}×{stageSize.h}（≤1K）。
                </div>
              </div>
            </div>
      ...
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-neutral-700">
          ...
          <li>有 reference：AI Comp 会分开发送：Image 1=画布导出（≤1K、无留白、比例=Gemini），Image 2=Reference（≤512）。</li>
