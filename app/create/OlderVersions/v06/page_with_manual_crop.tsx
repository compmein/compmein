...
function useHtmlImage(src?: string) {
  ...
  useEffect(() => {
    ...
    if (!cropTrRef.current || !cropRectRef.current) return;
    cropTrRef.current.nodes([cropRectRef.current]);
    cropTrRef.current.getLayer()?.batchDraw();
  }, [isCropMode]);

  function clampCropToImage(next: { x: number; y: number; width: number; height: number }) {
    if (!aiDraw) return next;
    const minW = 40;
    const minH = 40;

    const xMin = aiDraw.x;
    const yMin = aiDraw.y;
    const xMax = aiDraw.x + aiDraw.width;
    const yMax = aiDraw.y + aiDraw.height;

    let x = Math.max(xMin, Math.min(next.x, xMax - minW));
    let y = Math.max(yMin, Math.min(next.y, yMax - minH));
    let w = Math.max(minW, Math.min(next.width, xMax - x));
    let h = Math.max(minH, Math.min(next.height, yMax - y));

    return { x, y, width: w, height: h };
  }

  function applyCrop() {
    if (!aiImg || !aiDraw) return;
    const r = clampCropToImage(cropRect);

    const sx = aiImg.naturalWidth / aiDraw.width;
    const sy = aiImg.naturalHeight / aiDraw.height;

    const cropX = Math.round((r.x - aiDraw.x) * sx);
    const cropY = Math.round((r.y - aiDraw.y) * sy);
    const cropW = Math.max(1, Math.round(r.width * sx));
    const cropH = Math.max(1, Math.round(r.height * sy));
    ...
    ctx.drawImage(aiImg, cropX, cropY, cropW, cropH, 0, 0, cropW, cropH);
    const out = canvas.toDataURL("image/png");
    setAiOutputDataUrl(out);
    localStorage.setItem("lastAIOutputDataUrl", out);
    setIsCropMode(false);
  }
...
export default function CreatePage() {
  ...
  const STAGE_W = 900;
  const STAGE_H = 520;
  const CROP_W = STAGE_W;
  const CROP_H = STAGE_H;
  ...
  const aiImg = useHtmlImage(aiOutputDataUrl ?? undefined);
  const [isCropMode, setIsCropMode] = useState(false);
  const [cropRect, setCropRect] = useState({ x: 0, y: 0, width: 0, height: 0 });
  const [lastCombineHadReference, setLastCombineHadReference] = useState(false);

  const aiDraw = useMemo(() => {
    if (!aiImg) return null;
    return fitImage(aiImg, CROP_W, CROP_H);
  }, [aiImg, CROP_W, CROP_H]);

  // init crop rect when entering crop mode or when new AI image arrives
  useEffect(() => {
    if (!isCropMode) return;
    if (!aiDraw) return;

    const leftRatio = lastCombineHadReference ? STAGE_W / (STAGE_W + 280) : 1; // PANEL_W=280
    const w = aiDraw.width * leftRatio;
    setCropRect({
      x: aiDraw.x,
      y: aiDraw.y,
      width: w,
      height: aiDraw.height,
    });
  }, [isCropMode, aiDraw, lastCombineHadReference, STAGE_W]);
  ...
  async function runAdvancedCombine(modelType: "nano" | "pro") {
    ...
    const stage = stageRef.current;
    const hasReferenceNow = !!referenceDataUrl;
    setLastCombineHadReference(hasReferenceNow);
    if (!stage) return;
          ...
          <div className="lg:col-span-2 space-y-4">
                ...
                <div className="lg:col-span-2 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isCombining ? "AI 合成中..." : "Nano Banana"}
                      </button>
                      ...
                      >
                        {isCombining ? "AI 合成中..." : "Nano Banana Pro"}
                      </button>
                    </div>
            ...
            <div className="grid gap-4">
              <div className="rounded-3xl border bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-neutral-600">（AI 输出，可裁剪）</div>
                  </div>
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border bg-neutral-50">
                {aiOutputDataUrl ? (
                  isCropMode ? (
                    <Stage width={CROP_W} height={CROP_H}>
                      <Layer>
                        {aiImg && aiDraw && (
                          <KonvaImage
                            image={aiImg}
                  ...
                  <div className="flex h-[420px] items-center justify-center text-sm text-neutral-500">
                    还没有 AI 输出（直接运行 Advanced Combine；reference 可选）
                  </div>
                )}
              </div>
