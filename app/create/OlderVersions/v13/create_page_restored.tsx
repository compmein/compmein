...
/**
 * ✅ Single-file Create Page (Restored)
 * - Brings back: dropdown menus (subject & reference), "下载抠图", token badge, collapsible panels, IndexedDB saving for big AI output
 * - Removes: missing relative imports (./TokenBadge, ./useTokenBalance) to avoid build errors
 *
 * Next step (you asked later): replace the local token logic with DB-backed Supabase `user_tokens`.
 */
...
// ------------------------
// Minimal Token UI + local token hook (DB version comes next step)
// ------------------------
function TokenBadge({ balance }: { balance: number }) {
  return (
    <div className="rounded-xl border px-3 py-2 text-sm bg-white">
      <span className="text-neutral-500 mr-2">Tokens</span>
      <span className="font-semibold">{balance}</span>
    </div>
  );
}

function useTokenBalanceLocal() {
  const [balance, setBalance] = useState<number>(0);
  const [isReady, setIsReady] = useState(false);

  useEffect(() => {
    try {
      const v = localStorage.getItem("token_balance");
      setBalance(v ? parseInt(v, 10) : 10);
    } catch {
      setBalance(10);
    } finally {
      setIsReady(true);
  ...
  function hasTokens(cost: number) {
    return balance >= cost;
  }
  ...
  function spend(cost: number) {
    setBalance((b) => {
      ...
      try {
        localStorage.setItem("token_balance", String(nb));
      } catch {}
      return nb;
    });
  }

  return { balance, isReady, hasTokens, spend };
}
...
/**
 * ✅ localStorage 写保护（避免 QuotaExceededError）
 */
...
/** dataURL -> JPEG Blob（最长边不超过 maxSide） */
...
// ------------------------
// IndexedDB (save big AI output safely)
// ------------------------
...
export default function CreatePage() {
  ...
  // ✅ Tokens (local placeholder)
  const { balance: tokenBalance, isReady: tokenReady, hasTokens, spend } = useTokenBalanceLocal();
  ...
  async function onSelectPersonFile(file: File) {
    ...
    // 同时保存一份“未抠图人物”到 localStorage（小图才存，避免爆配额）
    try {
  ...
  /** ✅ AI Comp */
  async function runAiComp(modelType: "nano" | "pro") {
    const stage = stageRef.current;
    if (!stage) return;
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (safety.isSafetyBlock) {
          alert(
            `Pro Generation was blocked by the model safety filter.\n\n` +
              `Try:\n1) more neutral prompt (family-friendly, modest clothing)\n2) remove reference to isolate trigger\n\n` +
      ...
      console.error(e);
      alert(typeof e?.message === "string" && e.message.trim() ? e.message : "AI Comp 失败：请看 Console / Network");
    } finally {
  ...
  // 阴影定位：用 personRect 的脚底
  const shadowPos = useMemo(() => {
    if (!shadow.enabled || !personRect) return null;

    const footX = personRect.x + personRect.width / 2 + shadow.xOffset;
    const footY = personRect.y + personRect.height + shadow.yOffset;
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          ...
          人物不会自动抠图，点 <strong>“抠图”</strong> 才会请求 API。系统会保留你上一次抠过的人物、背景、reference（各 1 张）。
        </p>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            ...
            <details open className="rounded-2xl border p-4">
              ...
              <div className="mt-2 space-y-2">
                ...
                <div className="mt-2">
                  ...
                  />
                  <div className="mt-1 text-xs text-neutral-500">上传背景会自动保存 1 张（刷新后还在）。</div>
                </div>
              </div>
            </details>
            ...
            <details open className="rounded-2xl border p-4">
              ...
              <div className="mt-2 space-y-2">
                <select
                  className="w-full rounded-xl border px-3 py-2 text-sm"
                  value={subjectChoice}
                  onChange={(e) => setSubjectChoice(e.target.value as "uploaded" | "none")}
                >
                ...
                <div className="mt-2">
                  ...
                  <div className="mt-1 text-xs text-neutral-500">
                    上传人物会自动压缩到长边 1080p。PNG 会保留 PNG（方便抠图）。
                  </div>
                </div>
              ...
              <div className="mt-3 space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setShadow((s) => ({ ...s, squashY: parseFloat(e.target.value) }))}
                    className="w-full"
                    disabled={!shadow.enabled}
                  />
                </div>
            ...
            {/* 调色（保留 UI，暂不对人物做 Konva 自定义 filter；你之前那版如果要也能再合并回来） */}
            <details open className="rounded-2xl border p-4">
              <summary className="cursor-pointer select-none text-sm font-medium">调色（UI 保留）</summary>
              <div className="mt-3 space-y-3 text-sm">
                <div className="text-xs text-neutral-500">
                  你之前那套 Konva 调色 filter 我可以在下一步合并回来（不影响 dropdown/下载抠图）。
                  当前主要先把“你说丢的功能”全部恢复。
                </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex gap-4">
              <div className="flex-1">
                      ...
                      {shadowPos && (
                        <>
                          <Ellipse
                            ...
                            x={shadowPos.outer.x}
                            y={shadowPos.outer.y}
                            radiusX={shadowPos.outer.radiusX}
                            radiusY={shadowPos.outer.radiusY}
                            fill="black"
                            opacity={shadowPos.outer.opacity}
                            scaleY={shadowPos.outer.scaleY}
                            listening={false}
                            filters={[Konva.Filters.Blur]}
                            blurRadius={shadowPos.outer.blur}
                          />
                          <Ellipse
                            ref={innerShadowRef}
                            x={shadowPos.inner.x}
                            y={shadowPos.inner.y}
                            radiusX={shadowPos.inner.radiusX}
                            radiusY={shadowPos.inner.radiusY}
                            fill="black"
                            opacity={shadowPos.inner.opacity}
                            scaleY={shadowPos.inner.scaleY}
                            listening={false}
                            filters={[Konva.Filters.Blur]}
                            blurRadius={shadowPos.inner.blur}
                          />
                        </>
                      )}
                ...
                <div className="mt-2 text-xs text-neutral-500">
                  说明：背景等比缩放不拉伸；下载 Regular Comp 会自动去掉选框。
                </div>
              </div>

              {/* 右侧 reference 面板：只有选择 uploaded 才显示 */}
            ...
            <details open className="rounded-3xl border bg-white p-6 shadow-sm">
              ...
              <div className="mt-1 text-sm text-neutral-600">抠图是可选：想省 Token，可以直接用原图跑 AI Comp。</div>

              <div className="mt-3 grid gap-4 lg:grid-cols-3">
                <div>
                  <div className="mt-1 text-sm text-neutral-600">(Integrate char into the scene)</div>
                  <div className="mt-4 text-xs text-neutral-500">
                    Reference 可选：不选也能跑；选了会用于 clothing/object/pose。
                  </div>
                </div>
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border bg-neutral-50">
                {aiOutputDataUrl ? (
                  ...
                  <div className="flex h-[420px] items-center justify-center text-sm text-neutral-500">
                    还没有 AI 输出（运行 AI Comp；reference 可选）
                  </div>
                )}
              </div>
            </details>
      ...
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-neutral-700">
          <li>人物 dropdown：可选“暂无人物”（只合成背景）或“你上传的人物”。</li>
          <li>Reference dropdown：可选“暂无 Reference”或“你上传的 Reference”。</li>
          <li>“下载抠图”只会下载你最近一次抠图的 PNG（不会扣 Token）。</li>
        </ul>
      </div>
    </div>
  );
