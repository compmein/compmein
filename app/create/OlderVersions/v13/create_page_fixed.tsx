...
/**
 * NOTE (why this file builds now):
 * - Removed relative imports like "./TokenBadge" and "./useTokenBalance"
 * - Implemented a tiny in-file TokenBadge + useTokenBalanceLocal to avoid module-not-found.
 * - This keeps your UI working immediately; next step is wiring this to your Supabase `user_tokens` table.
 */
...
/**
 * TEMP (local) token hook to avoid build error.
 * Next step: replace with real DB-backed token reads/writes via Supabase.
 */
...
async function resizeImageFile(file: File, maxLongSide = 1080, quality = 0.9): Promise<File> {
  ...
  return new File([blob], file.name.replace(/\.\w+$/, outType === "image/png" ? ".png" : ".jpg"), {
    type: outType,
  });
}
...
export default function CreatePage() {
  ...
  // Tokens (local placeholder for now)
  const { balance: tokenBalance } = useTokenBalanceLocal();
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        {/* Header: title + token + email + sign out */}
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {userEmail && (
              <div className="hidden sm:block text-xs text-neutral-500 max-w-[220px] truncate">
                {userEmail}
              </div>
            )}
      ...
      <div className="rounded-3xl border bg-white p-6 shadow-sm">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-neutral-700">
          ...
          <li>
            有 reference：Advance Combine 会把「客户合成截图 + reference 面板」拼成 1 张图发给 NanoBanana；没 reference 就只发客户合成截图。
          </li>
