...
Do not include the reference image or any UI elements in the final image.
...
/** DataURL -> 压缩 JPEG Blob（用于发给 AI，省钱 & 更稳） */
...
/** Reference 专用：强制限制宽高（<= 512w 且 <= 768h） */
async function dataUrlToSmallRefJpegBlob(dataUrl: string, maxW = 512, maxH = 768, quality = 0.85): Promise<Blob> {
  const img = await loadImageFromDataUrl(dataUrl);
...
export default function CreatePage() {
  ...
  async function onSelectReferenceFile(file: File) {
    // reference 先压小，避免浪费 token（同时限制高度/宽度）
    const resized = await resizeImageFile(file, 768, 0.88);
    const dataUrl = await blobToDataURL(resized);
  ...
  /** ✅ AI Comp（Nano Banana）：可选 reference（有就当衣服/配饰参考） */
  async function runAdvancedCombine(modelType: "nano" | "pro") {
    ...
    try {
      ...
      // 2) 准备发给 Nano Banana 的两张图：主图 +（可选）reference
      // 主图：从 stage 导出，压到 1K 左右（长边<=1024）
      const sceneBlob = await dataUrlToJpegBlob(stageDataUrl, 1024, 0.85);

      // reference：强制限制尺寸（<=512w 且 <=768h），省钱
      const refBlob = referenceDataUrl ? await dataUrlToSmallRefJpegBlob(referenceDataUrl, 512, 768, 0.85) : null;

      // 3) 调后端
      const fd = new FormData();
      fd.append("image", sceneBlob, "scene.jpg");
      if (refBlob) fd.append("refImage", refBlob, "reference.jpg");
